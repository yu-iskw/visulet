...
describe('executeCli', () => {
  ...
  it('inspects, renders, compiles, and reports capabilities', () => {
    ...
    expect(executeCli({ command: 'render', document, format: 'svg', json: true }).stdout).toContain('<svg');
    expect(executeCli({ command: 'render', document, format: 'png', json: true }).exitCode).toBe(1);
    expect(executeCli({ command: 'compile', document, backend: 'svg', json: true }).exitCode).toBe(0);
    expect(executeCli({ command: 'compile', document, backend: 'vega-lite', json: true }).exitCode).toBe(0);
    expect(executeCli({ command: 'compile', document, backend: 'unknown', json: true }).exitCode).toBe(1);
    expect(executeCli({ command: 'capabilities', json: true }).stdout).toContain('svg');
    expect(executeCli({ command: 'capabilities', backend: 'mermaid', json: true }).exitCode).toBe(0);
    expect(executeCli({ command: 'capabilities', backend: 'vega-lite', json: true }).exitCode).toBe(0);
    ...
    expect(executeCli({ command: 'compile', document: flow, backend: 'mermaid', json: true }).exitCode).toBe(0);
    expect(executeCli({ command: 'compile', document: flow, backend: 'mermaid', json: false }).stdout).toContain(
      'flowchart',
    );
    expect(executeCli({ command: 'validate', document, json: false }).stdout).toContain('valid');
    expect(executeCli({ command: 'inspect', document: { version: '0', views: [], extra: true }, json: true }).exitCode).toBe(1);
    expect(executeCli({ command: 'patch', document, patch: { op: 'replace' }, json: true }).exitCode).toBe(1);
    expect(
      executeCli({ command: 'compile', document: { version: '0', views: [] }, backend: 'mermaid', json: true }).exitCode,
    ).toBe(1);
  });
